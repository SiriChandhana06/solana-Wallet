...
describe('solana', () => {
  ...
  it('Initialize Solana', async () => {
    ...
    const currentCount = await program.account.solana.fetch(
  ...
  it('Increment Solana', async () => {
    await program.methods
      .increment()
      .accounts({ solana: solanaKeypair.publicKey })
      .rpc();
  ...
  it('Set close the solana account', async () => {
    ...
    const userAccount = await program.account.solana.fetchNullable(
      solanaKeypair.publicKey
    );
