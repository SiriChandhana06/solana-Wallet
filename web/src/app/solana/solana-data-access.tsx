import { getSolanaProgram, getSolanaProgramId } from '@solana/anchor';
...
export function useSolanaProgram() {
  ...
  const programId = useMemo(
    () => getSolanaProgramId(cluster.network as Cluster),
    [cluster]
  );
  ...
  const initialize = useMutation({
    ...
    mutationFn: (keypair: Keypair) =>
      program.methods
        .initialize()
        .accounts({ solana: keypair.publicKey })
        .signers([keypair])
        .rpc(),
    onSuccess: (signature) => {
...
export function useSolanaProgramAccount({ account }: { account: PublicKey }) {
  ...
  const closeMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.close().accounts({ solana: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accounts.refetch();
    },
  ...
  const decrementMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.decrement().accounts({ solana: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
  ...
  const incrementMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.increment().accounts({ solana: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accountQuery.refetch();
    },
  ...
  const setMutation = useMutation({
    ...
    mutationFn: (value: number) =>
      program.methods.set(value).accounts({ solana: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accountQuery.refetch();
    },
